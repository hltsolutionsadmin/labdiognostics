...
export class TestsPageComponent {
  ...
  private readonly _productsDebug$ = this.products$.pipe(
    tap((arr) => {
      console.debug('[TestsPageComponent] products$ next', {
    ...
    tap({
      complete: () =>
        console.debug('[TestsPageComponent] products$ complete', {
          instanceId: this._instanceId,
          time: new Date().toISOString()
        })
    })
  ...
  readonly categories = computed(() => {
    console.debug('[TestsPageComponent] categories() recompute', {
      instanceId: this._instanceId,
      time: new Date().toISOString(),
  ...
  readonly filtered = computed(() => {
    console.debug('[TestsPageComponent] filtered() recompute', {
      instanceId: this._instanceId,
      time: new Date().toISOString(),
